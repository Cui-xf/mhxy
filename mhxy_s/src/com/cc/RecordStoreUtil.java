...
public final class RecordStoreUtil {
    public static void save(byte[] value, String key) {

        boolean contained = containRecordStore(key);
        RecordStore var2 = null;
        try {
            var2 = openRecordStore(key, contained);
            var2.addRecord(value, 0, value.length);
            return;
    ...
    public static byte[] getRecord(String var0) {
        if (containRecordStore(var0)) {
            RecordStore var1 = null;

            try {
                if ((var1 = openRecordStore(var0, false)).getNumRecords() + 1 <= 1) {
                    return null;
                }
    ...
    private static RecordStore openRecordStore(String key, boolean reset) throws RecordStoreException {
        if (reset) {
            RecordStore.deleteRecordStore(key);
        }
        return RecordStore.openRecordStore(key, true);
    }

    private static boolean containRecordStore(String key) {
        String[] recordStores = RecordStore.listRecordStores();
        if (recordStores != null) {
            for (int var2 = recordStores.length - 1; var2 >= 0; --var2) {
                if (recordStores[var2].equals(key)) {
                    return true;
                }
            }
