...
### 2.9 任务追踪与 NPC 可选任务（`g(DataInputStream)`，`bt.java:1413`）

收包 **8207**（`q.java` `case 8207`，非战斗且非 PC 模拟器分支）调用本方法，写入「追踪串 + 与 NPC 交互用的任务 id 列表」等。

- `bt.bQ`：String，任务追踪总述（`readUTF`）
- `bt.bI`：int[]，任务 ID（与 NPC 菜单选中项、`4107` 上行一致）
- `bt.bJ`：String[]，任务名称
- `bt.nI`：short[]，与 `bI` 下标对齐的附加 short（等级/类型等，见 `bt.g` 循环）
- `bt.bK`：byte[]，任务状态

### 2.10 任务列表快照（`h(DataInputStream)`，`bt.java:1445`）

> **更正（2026-03-29）**：旧版误将本小节标为「副本/战斗」。根据 `log/任务&&战斗日志.out` 与 `ao.x()` / `8223` 逻辑，**`bt.h` 解析的是服务端下发的任务列表**（两组并行数组），用于界面「已接 / 剧情 / 活动…」及 `5124/5125/4132/4133` 等发包中的 `bt.bR[下标]`。战斗过程**不**由 `h()` 驱动。

收包 **8223**（`q.java` `case 8223`）调用 `bt.h`。

**第一组（主列表，常用）**

- `bt.bR`：int[]，任务 ID
- `bt.bS`：String[]，任务名称
- `bt.bT`：short[]，展示用等级（界面拼「×级」）
- `bt.bU`：byte[]，状态枚举
- `bt.bV`：String[]，目标/详情（如「目标：消灭5只树精，5只草精」）

**第二组（扩展列表）**

- `bt.bL`：int[]，`bt.bM`：String[]，`bt.bN`：short[]，`bt.bO`：int[]，`bt.bP`：String[]

**战斗相关收包**见 `q.java`：**8206**（`q.b`，回合同步）、**8217**（`q.e`，`bp[]` 开战）、**8218**（阶段/结束）等；详述见 `docs/任务与战斗系统设计分析.md`、`docs/网络包协议分析.md` 包表。
